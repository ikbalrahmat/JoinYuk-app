...
<head>
    ...
    <title>Reset Password - {{ config('app.name', 'JoinYuk') }}</title>
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 lg:p-16 xl:p-24 bg-white relative">
        ...
        <div class="w-full max-w-md mx-auto">
            ...
            <div class="mb-10 text-center lg:text-left mt-8 lg:mt-0">
                <h2 class="text-3xl font-bold text-slate-900 mb-3 tracking-tight">Buat Password Baru</h2>
                <p class="text-slate-500 text-base">Silakan buat password baru untuk akun Anda. Pastikan kombinasi password kuat dan mudah diingat.</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.update') }}" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input id="email" type="email" name="email" value="{{ $email ?? old('email') }}" required autocomplete="email" autofocus readonly
                            class="block w-full pl-11 pr-4 py-3.5 bg-slate-100 border border-slate-200 rounded-2xl text-sm outline-none font-medium text-slate-600"
                ...
                <div class="space-y-2">
                    <label for="password-confirm" class="block text-sm font-semibold text-slate-700">{{ __('Konfirmasi Password Baru') }}</label>
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors group-focus-within:text-primary-500 text-slate-400">
                            <i class="fa-solid fa-lock"></i>
                        </div>
