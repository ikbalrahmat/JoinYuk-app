...
<div class="-m-6 h-[calc(100vh-64px)] flex flex-col overflow-hidden bg-white font-sans border-0">
    ...
    <div class="bg-white border-b border-gray-200 px-6 py-3 flex justify-between items-center z-20 shrink-0">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-lg font-bold text-gray-800 leading-tight truncate max-w-sm" id="topbar_form_name">{{ $presence->nama_kegiatan }}</h2>
                <p class="text-[11px] text-gray-500 uppercase tracking-wider font-bold" id="topbar_form_type">{{ $presence->tipe_form === 'card' ? 'Card Form' : 'Standard Form' }}</p>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <button type="button" id="btn-preview" onclick="previewForm()" class="px-4 py-2 text-sm font-bold text-primary-600 bg-primary-50 border border-primary-200 hover:bg-primary-100 hover:text-primary-700 rounded-lg shadow-sm transition flex items-center gap-2">
                <i class="fa-solid fa-eye"></i> Preview
            </button>
            <button type="button" id="btn-akses" onclick="aksesForm()" class="px-5 py-2 text-sm font-bold text-white bg-slate-800 hover:bg-slate-900 rounded-lg shadow-sm transition flex items-center gap-2">
    ...
    <div class="flex flex-1 overflow-hidden relative">
        ...
        <div class="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar relative bg-[#eef2f6]" id="canvasScrollArea" onclick="closeProperties(event)">
            ...
            <form action="{{ route('presence.update', $presence->id) }}" method="POST" id="mainForm">
                @csrf
                @method('PUT')
                ...
                <input type="hidden" name="form_type" id="db_form_type">
        ...
        <div id="propertiesPanel" class="w-[400px] bg-white border-l border-gray-200 flex flex-col shrink-0 shadow-[-10px_0_20px_rgba(0,0,0,0.04)] z-20 absolute right-0 top-0 bottom-0 transform translate-x-full transition-transform duration-300">
            ...
            <div class="absolute bottom-0 left-0 w-full p-4 border-t border-gray-200 bg-white flex justify-end gap-3 shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
                ...
                <button type="button" id="btn-save-props" onclick="saveAndCloseProperties()" class="px-8 py-2 text-sm font-bold text-white bg-teal-600 hover:bg-teal-700 rounded-full shadow-sm transition">
                    Save
                </button>
            </div>
        </div>
...
<script>
    @php
        $initialFields = is_string($presence->custom_fields) ? json_decode($presence->custom_fields, true) : $presence->custom_fields;
        $initialHeaderConfig = is_string($presence->header_config) ? json_decode($presence->header_config, true) : $presence->header_config;
        
        // Fallback for old templates without header_config
        if (!$initialHeaderConfig) {
            $initialHeaderConfig = [
                'show_date' => !empty($presence->tgl_kegiatan),
                'show_time' => !empty($presence->tgl_kegiatan),
                'show_location' => !empty($presence->tempat),
                'date' => $presence->tgl_kegiatan ? \Carbon\Carbon::parse($presence->tgl_kegiatan)->format('Y-m-d') : '',
                'time' => $presence->tgl_kegiatan ? \Carbon\Carbon::parse($presence->tgl_kegiatan)->format('H:i') : '',
                'location' => $presence->tempat ?? ''
            ];
        }
    @endphp
    ...
    let selectedFormType = '{{ $presence->tipe_form }}';
    
    // State Khusus Header
    const savedHeaderConfig = {!! json_encode($initialHeaderConfig) !!};
    
    let headerData = {
        title: '{{ $presence->nama_kegiatan }}',
    ...
    function buildPreviewCardHTML(field, isActive) {
        ...
        if (isActive) {
            actionTools = `
                ...
                <div class="absolute -right-[46px] top-1/2 -translate-y-1/2 flex flex-col shadow-[0_4px_20px_rgba(0,0,0,0.15)] rounded-lg overflow-hidden border border-slate-700 bg-slate-900 z-10">
    ...
    function saveAjax(callback) {
        ...
        fetch('{{ route('presence.update', $presence->id) }}', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        .catch(err => {
            console.error(err);
            if(callback) callback(); // Lanjut aja meskipun error jaringan, biar preview kebuka
        });
    }
    ...
    function previewForm() {
        ...
        saveAjax(() => {
            ...
            window.open('{{ route('absen.index', $presence->slug) }}', 'preview_window', 'width=800,height=900,scrollbars=yes');
        });
    }
    
    function aksesForm() {
        const btn = document.getElementById('btn-akses');
        const originalText = btn.innerHTML;
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Loading...';
        
        saveAjax(() => {
            btn.innerHTML = originalText;
            window.open('{{ route('absen.index', $presence->slug) }}', '_blank');
        });
    }

    function saveAndCloseProperties() {
    ...
    // ========== SUBMIT KE BACKEND (FULL PAGE RELOAD) ==========
    function submitForm() {
        updateHiddenInputs();
